...
int main(int argc, char *argv[]) {
    ...
    if (info_file == NULL) {
        perror("Erreur: impossible de lire le fichier d'information du serveur");
        printf("Assurez-vous que le serveur est démarré\n");
        exit(EXIT_FAILURE);
    }
    
    if (fscanf(info_file, "%s %d", server_address, &server_port) != 2) {
        fprintf(stderr, "Erreur: format du fichier d'information invalide\n");
        fclose(info_file);
        exit(EXIT_FAILURE);
    }
    fclose(info_file);
    
    printf("Serveur trouvé: %s:%d\n", server_address, server_port);
    ...
    do {
        ...
        n = rand() % NMAX + 1;
        printf("\n═══════════════════════════════════════════\n");
        printf("Requête vers %s:%d\n", server_address, server_port);
        printf("Envoi du nombre n=%d au serveur...\n", n);
